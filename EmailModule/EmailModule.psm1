...
Import-LocalizedData -BindingVariable LocalizedStrings -FileName EmailModule.strings.psd1

function Get-Banner{
    write-host '   ____|                   _)  |   \  |             |         |       '
    write-host '   __|    __ `__ \    _` |  |  |  |\/ |   _ \    _` |  |   |  |   _ \ '
    write-host '   |      |   |   |  (   |  |  |  |   |  (   |  (   |  |   |  |   __/ '
    write-host '  _____| _|  _|  _| \__,_| _| _| _|  _| \___/  \__,_| \__,_| _| \___| '

    write-host -no $LocalizedStrings.ref_general_help
    write-host -no " "
    write-host -fore Yellow $LocalizedStrings.ref_GetHelp

    write-host -no $LocalizedStrings.ref_help_for_cmdlet
    write-host -no " "
    write-host -fore Yellow $LocalizedStrings.ref_GetHelpCMDLET
}
Get-Banner
...
`EmailCommands` (defined in the module's .\EmailModule\lib\Core\EmailLibrary.dll or 
.\EmailModule\lib\desktop\EmailLibrary.dll) handles message creation, authentication, and
secure transmission using STARTTLS.
...
EmailLibrary.dll assembly that contains the `EmailCommands` class andstatic method
...
.COMPONENT
Email Module – Core Messaging Functionality

.ROLE
Provides SMTP-based email delivery for automation, notifications, and alerting.

.FUNCTIONALITY
Sends authenticated and encrypted SMTP messages using .NET assemblies (MimeKit and MailKit).

.LINK
https://github.com/Brandon-J-Navarro/Powershell_Email-Module
https://www.powershellgallery.com/packages/EmailModule/
